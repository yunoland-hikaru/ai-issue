...
- 記事は**原文の事実(ファクト)だけ抽出→AI issue独自文体で新規作成した独立著作物**（原文の文体・構成をなぞらない=著作権配慮）。文体は**中立・簡潔・逆ピラミッド・過張/推測表現禁止**＋**読みやすさ優先（接続詞で文を自然につなぎ細切れにしない、1文40〜60字、本文4〜6段落500〜800字）**（`lib/claude.ts`のsystemプロンプトに「AI issue 文体ガイドライン」を内蔵。原本は `C:\work\ai-issue-style-guide.md`、リポ外）。タイトルは必ず30字以内。事実/数値/引用は捏造禁止。max_tokens 生成4096/翻訳8192。
