...
3. **ja記事を即 insert**（画像・翻訳より前。created_at = 生成時刻、ロゴ=`logoUrlForDomain`、image_url=null、ko/en=null）。以降が遅延/失敗しても記事は残り、Sonnet生成を無駄にしない。
4. 画像（best-effort、insert後の UPDATE。著作権クリーン、RSS元画像は使わない）:
   - `searchStockImage`（lib/stock.ts、Pexels、image_keywordsで検索）→ 当たればその写真
   - 無ければ `generateImage`（lib/openai.ts、gpt-image-1、image_prompt）
   - Supabase Storage へ保存 → image_url を UPDATE（失敗しても記事は残る＝migrate-imagesが補完）

**翻訳(ko/en)は collect では行わない**。Vercel Hobby の 60秒制限に収めるため、`generateArticle`(Sonnet ~35s)＋`translateArticle`(Haiku ~25s)の直列で 60秒超過→504になっていた問題への対処。ko/en は後追いの `migrate-translations` が埋める。**collect.yml では collect の直後に同 run で `migrate-translations?limit=2` を呼ぶ**ので、新規記事(jaのみ)も最新順スキャンで最優先に拾われ数十秒で ko/en が付く（恒久的な ja-on-ko は発生しない）。画像の保険として `migrate-images?limit=1` も同 run で呼ぶ。
...
翻訳漏れの後追い修正: `POST /api/migrate-translations?limit=1..20`（ko/en訳に日本語の文字＝かな・漢字が残った記事＝`hasJpResidue`で検出を、ja原文から再翻訳して上書き。直近200件を走査し対象をlimit件まで。本文ja・画像等は変更しない。haikuの訳し漏れで韓国語に「研究/開発」等が残りモバイルで横スクロールした事象の救済用）。
...
- ホーム（`app/[lang]/page.tsx`）: **サーバーコンポーネント**（ISR `revalidate=3600`＝コスト対策。ただし新着は下記の**オンデマンド再検証**で即反映されるので時間ベースは1時間=安全網で十分）で最新50+人気10をSupabaseから取得→`HomeView.tsx`(client)に渡す（初期HTMLに記事を含めSEO対応。HomeViewはinitialのみ使用＝クライアント追加fetchなし）。最新生成記事が大きいヒーロー + 残りはリスト。記事0件なら「ただいま準備中です」空状態（ダミー記事は出さない）。タブ切替は取得済み記事のクライアント絞り込み。
  - **オンデマンド再検証**: 時間ベースISRだけだと新着が最大revalidate秒・ロケール別にバラついて反映され「F5で出ない/言語切替で出たり消えたり/タブ開き直すと出る」現象になる（`/ja` `/ko` `/en`は別々のISRキャッシュ）。対策として `collect`(insert後=3ロケール)・`migrate-translations`(ko/en埋めた後=ko/enのみ)・`migrate-images`(画像埋めた後=3ロケール)が `revalidatePath('/ja'|'/ko'|'/en')` でホームを即無効化する。これにより時間ベースの revalidate を長く（3600秒）でき、ISR Writes を抑えつつ新着は即反映できる。
  - **ISR Writes 無料枠（重要・運用）**: Vercel無料枠は **ISR Writes 20万/月**。超えると**プロジェクトが自動pause**（2026-06に到達警告メール）。原因は旧 home `revalidate=60`（3ロケール×毎分再生成で月13万超）。→ 600→3600秒へ緩和＋オンデマンド再検証で時間ベース書き込みを大幅削減（home時間ベース ≈ 72回/日）。月初リセット。今後さらに低revalidate値を増やさないこと。
...
- 記事は**原文の事実(ファクト)だけ抽出→AI issue独自文体で新規作成した独立著作物**（原文の文体・構成をなぞらない=著作権配慮）。文体は**中立・簡潔・逆ピラミッド・過張/推測表現禁止**＋**「やさしく・読みやすく」最優先**（仕様書/論文調にせず専門用語・数値の意味を噛み砕いて説明。接続詞で文を自然につなぎ細切れにしない、1文40〜60字、本文6〜7段落1400〜2000字）（`lib/claude.ts`のsystemプロンプトに「AI issue 文体ガイドライン」を内蔵。原本は `C:\work\ai-issue-style-guide.md`、リポ外）。タイトルは20〜25字目安・必ず30字以内。事実/数値/引用は捏造禁止。max_tokens 生成6144/翻訳12000。
  - **本文は単なる事実要約で終わらせず「背景・文脈」＋「意味づけ（なぜ重要か）」を必須化**（構成: 結論→背景・文脈→詳細→意味づけ）。AdSense「low value/scaled content」対策＝記事ごとの独自価値を高めるため。ただし具体的な数値・固有名詞・発言は原文で確認できたものだけ（背景/意味づけはAI分野の一般前提と事実から導ける範囲・控えめな表現に限定、未確認の数値・予測値は作らない）。
- 自社の独立著作物という立場を明確化: **原文出典は記事ページに表示しない**（source_url はDB保存=重複判定用のみ）。記事末尾の著作権表記(`ArticleView.tsx` creditNote)と `/editorial` は「AI issueが事実から独自作成した著作物・無断転載/再配布/AI学習利用を禁止」と強気に明記（旧「原文出典を確認」表記は廃止）。
- 翻訳の固有名詞: **韓国語訳は企業/製品/サービス/モデル名をラテン文字のまま残す**（Microsoft/Copilot/DeepSeek/OpenAI等。ハングル音訳しない）。一般名詞(인공지능/반도체等)はハングル。日本語文字を残さないルールは継続(`lib/claude.ts` translateArticle)。
- 言語切替で新着が出ない問題への対処: `LangContext` の setLang で `router.push` 後に **`router.refresh()`** を呼び、クライアントRouter Cacheの古いロケールRSCを破棄して最新を取り直す。
...
- 多言語: ko/en は collect 直後に `migrate-translations` が埋める（collect 自体はjaのみ保存＝60秒制限対策、上記フロー参照）。UIはlangで出し分け、未訳（埋まる前の数十秒）はjaフォールバック。
...
- RSS出典は独立系AIメディア**6社**: 0=TechCrunch / 1=VentureBeat(`/feed` ※末尾スラッシュ無し。`/feed/`は308) / 2=The Decoder / 3=Synced / 4=AI Business / 5=InfoQ（`lib/rss.ts`）。記事は**事実だけ抽出→独自文体で新規作成**前提なので事実利用は可。出典を複数にして同一ソース連続収集(=出典推測)も回避。**除外**: AI Times(「AI学習・活用禁止」明記、過去記事削除済) / The Verge=Vox・Condé Nast・MIT TR等(AIに強硬な大手)。403で弾く媒体(MarkTechPost/Unite.AI等)も不可。rss-parserにUA設定済。ソース増減時は collect.yml/route.ts の `RANDOM % N`(現在6)・コメント・index範囲も揃える。
