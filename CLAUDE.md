...
- RSS出典は一般メディア3社: 0=TechCrunch / 1=VentureBeat(`/feed` ※末尾スラッシュ無し。`/feed/`は308) / 2=The Verge（`lib/rss.ts`）。記事は**事実だけ抽出→独自文体で新規作成**前提なので事実利用は可。ただし**「AI学習・活用禁止」明記のAI Timesは除外**(過去記事も削除済)。The Verge(Vox)はAIスクレイピングに強硬な点は留意。rss-parserにUA設定済。ソース増減時は collect.yml/route.ts の `RANDOM % N`・コメント・index範囲も揃える（現在3）。
