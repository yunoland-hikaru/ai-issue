...
7. X(Twitter)自動ポスト（`lib/x.ts`、`postArticleToX`）: insert成功直後に「英語タイトル+カテゴリ別ハッシュタグ+`/en/news/[id]`リンク」をツイート。OAuth1.0a(ボット垢トークン)。env未設定なら自動skip、失敗してもcollectは継続(results.errorsに記録)。リンクのOG/Twitterカードで画像が自動表示される。
...
- **多言語ルーティングは「パスベース」(`/ja` `/ko` `/en`)**（SEO用にロケール別URLを分離。旧「単一URL＋Cookie/geoで出し分け」から移行済み）。
  ...
  - **`proxy.ts`（リポジトリ直下＝このNext16ではmiddlewareの新名称）** がロケール接頭辞の無いリクエストを検出→`/ja|ko|en`へ307リダイレクト。優先順: Cookie `lang`（手動選択）→ `x-vercel-ip-country`（KR→ko/JP→ja）→ `Accept-Language` → 既定`ja`(=x-default)。matcherで`_next`・`api`・拡張子付きファイルは除外。旧`/news/[id]`共有リンクも自動リダイレクトで保存される。
  ...
  - 注意: **ページ移動後は`.next`キャッシュを削除してからビルド**（`.next/dev/types/validator.ts`が旧パスを参照して型エラーになる）。ローカルはgeoヘッダ無し→Accept-Language/ja。
...
X(Twitter)自動ポスト用(OAuth1.0a): `X_API_KEY`, `X_API_SECRET`, `X_ACCESS_TOKEN`, `X_ACCESS_SECRET`。※未設定でもcollectは動く（ツイートをskip）。アプリ権限はRead and Write。無料枠で月~360件想定。
