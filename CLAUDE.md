...
- ホーム（`app/[lang]/page.tsx`）: **サーバーコンポーネント**（ISR `revalidate=600`＝コスト対策。ただし新着は下記の**オンデマンド再検証**で即反映）で最新50+人気10をSupabaseから取得→`HomeView.tsx`(client)に渡す（初期HTMLに記事を含めSEO対応。HomeViewはinitialのみ使用＝クライアント追加fetchなし）。最新生成記事が大きいヒーロー + 残りはリスト。記事0件なら「ただいま準備中です」空状態（ダミー記事は出さない）。タブ切替は取得済み記事のクライアント絞り込み。
  - **オンデマンド再検証**: ISR 600秒だけだと新着が最大10分・ロケール別にバラついて反映され「F5で出ない/言語切替で出たり消えたり/タブ開き直すと出る」現象になる（`/ja` `/ko` `/en`は別々のISRキャッシュ）。対策として `collect`(insert後)・`migrate-translations`(ko/en埋めた後)・`migrate-images`(画像埋めた後)が `revalidatePath('/ja'|'/ko'|'/en')` で3ロケールのホームを即無効化する。再生成は新着が出た時だけ（≈900回/月）なので無料枠に対し無視できる。revalidate=600は安全網として残す。
