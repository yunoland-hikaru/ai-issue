...
条件：
- title_ja: 記事の核心を必ず全角30字以内で（厳守）。30字を超える場合は二次的要素＝パラメータ数・性能数値・ライセンス名・バージョン番号・補足修飾を削り、「主語＋核心の動作」だけに絞る。疑問形(「〜か?」)は避ける
  - 悪い例（長すぎ・要素過多）:「Z.ai、7530億パラメータの『GLM-5.2』をMITライセンスで公開」
  - 良い例（核心だけ）:「Z.ai、オープンLLM『GLM-5.2』を公開」「OpenAI、推論特化モデル『o3』を正式リリース」
- content_ja: 抽出した事実だけで新規作成した本文。4〜6段落・合計500〜800字程度、各段落を<p>タグで区切る。各段落は3〜5文。文と文は接続詞で自然につなぎ、細切れの短文を並べない（流れるように読ませる）。リード文は入れず本文から始める（リードはsummary_jaが担う）。原文にない数値・引用を捏造しない。原文リンクは含めない
