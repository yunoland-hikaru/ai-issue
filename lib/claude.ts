...
- content_ja: 抽出した事実だけで新規作成した本文。5〜6段落・合計1200〜1800字程度、各段落を<p>タグで区切る。各段落は3〜5文。文と文は接続詞で自然につなぎ、細切れの短文を並べない（流れるように読ませる）。専門用語や数値を羅列せず、その意味や背景をやさしく補いながら書く。リード文は入れず本文から始める（リードはsummary_jaが担う）。原文にない数値・引用を捏造しない。原文リンクは含めない
